...
def zapujceni_auta():
    ...
    if zapujceni_auta == "Peugeot":
        print(Auto.get_info(peugeot))
        print(Auto.pujc_auto(peugeot))
    elif zapujceni_auta == "Škoda":
        print(Auto.get_info(skoda))
        print(Auto.pujc_auto(skoda))
    else:
        print("Omlouváme se, ale toto auto v nabídce nemáme.")

zapujceni_auta()

uzivatel = Auto.vrat_auto

print(Auto.vrat_auto(uzivatel, stav_tachometru = input("Jaký máte stav na tachometru? "), pocet_dni = int(input("Kolik dní jste měli auto půjčené? "))))
